...
export async function POST(req: NextRequest) {
  ...
  // Looked up server-side with the service role key so the mentor's email
  // is never sent to or queried from the browser.
  ...
  const subject = `Peer Tutoring session booked — ${slot}`;
  ...
  const studentHtml = `
    ...
    <p style="background:#f4f4f0;padding:12px 16px;border-radius:8px">
      Please discuss where to meet for the session and don't forget to rate your mentor afterwards.
    </p>
